...
export function checkIntegrity(INTENTS, PROJECTIONS, ONTOLOGY) {
  ...
  // === 6. Анкеринг — через checkAnchoring (v1.8, §15) ===
  // Конструктивные частицы (entity, effect.target base) → error, блокируют crystallizeV2.
  // Описательные (field, witness, condition) → warning/info, soft.
  const anchoring = checkAnchoring(INTENTS, ONTOLOGY);
  issues.push(...anchoring.errors, ...anchoring.warnings, ...anchoring.infos);
