...
`@intent-driven/effect-runner-http@0.1.0` (BSL-1.1) — generic HTTP effect-runner для IDF. Преобразует intents в fetch-запросы через CRUD-conventions (POST/PATCH/DELETE/GET с `:id` path-params). Public API: `runIntent()` (pure async) + `useHttpEngine()` React hook. `inferEndpoint` inferrит HTTP method + path из `intent.alpha` + entity-name; `intent.endpoint` override'ит inference. Auth через sync/async `getAuthToken`.
